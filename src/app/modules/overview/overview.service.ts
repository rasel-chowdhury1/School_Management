...
today.setHours(0, 0, 0, 0);
  ...
  const attendanceStats = await ClassRoutine.aggregate([
   ...
   const assignmentDue =   await Assignment.countDocuments({
  ...
  //   //added total upcoming exams count
   const totalUpcomingExams = await Exam.countDocuments({
      teacherId: new mongoose.Types.ObjectId(user.teacherId),
      date: { $gt: today }
    });


    // -------------------------------------
    // TOTAL MY UNREAD MESSAGE COUNT (Robust)
    const userId = new mongoose.Types.ObjectId(String(user.userId));
    
    
  // Get all conversation IDs where user is a participant
  const myConversationIds = await Conversation.find({ users: userId }).distinct("_id");

const totalUnreadMessageCountResult = await Message.aggregate([
  {
    $match: { conversationId: { $in: myConversationIds } }
  },
  {
    $addFields: {
      isUnreadForMe: {
  ...
  },
  {
    $group: {
      _id: null,
      total: { $sum: { $cond: ['$isUnreadForMe', 1, 0] } }
    }
  }
]);

// শুধু value নিতে
const totalUnreadMessageCount = totalUnreadMessageCountResult[0]?.total || 0;
...
console.log({ totalUnreadMessageCount });
  ...
  return {
    ...
    overallAttendanceRate,
    activeStudents,
    assignmentDue : assignmentDue || 0, // remove old logic
    totalUpcomingExams:  totalUpcomingExams || 0, // adjust as needed
    totalSummoned,
    totalTerminated,
    supervisorClasses,
